...
| `OPEN_NOTEBOOK_ENABLE_LOCAL_VAULTS` | No | `false` | Enables the **shelved** server-side disk vault model (the server reads/watches a mounted folder). Off by default — the supported path is the [Obsidian push plugin](../../notebook_obsidian/README.md). While off, the disk browse/validate/link/refresh endpoints return `410`, the filesystem watcher doesn't start, and the folder-picker UI is hidden. |
