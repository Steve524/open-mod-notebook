...
export class SyncEngine {
	...
	async start(): Promise<void> {
		this.setStatus("syncing", "Connecting to Open Notebook…");
		...
		if (upserts.length > 0) {
			...
			for (const item of upserts) {
				...
				try {
					...
					console.error("Open Notebook: failed to build payload", item.path, err);
					retry.push(item);
				}
			}
	...
	private async handleVaultNotRegistered(err: unknown): Promise<boolean> {
		...
		console.warn("Open Notebook: vault not registered, re-connecting before retry", err);
		await this.ensureConnected();
		return true;
	}
	...
	private classify(err: unknown): "ack" | "retry" | "drop" | "stop" {
		...
		if (err instanceof PermanentError) {
			console.warn("Open Notebook: permanent error, dropping batch", err);
			new Notice(`Open Notebook: ${err.message}`);
			return "drop";
		}
		console.error("Open Notebook: unknown error", err);
		return "retry";
	}
