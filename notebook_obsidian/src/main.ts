...
import { NotebookApiClient } from "./api-client";
...
import {
	...
	type QueueItem,
	type StatusState,
	type NotebookSyncSettings,
} from "./types";
import { generateVaultUuid } from "./vault-identity";

/** Open Notebook plugin entry point. */
export default class NotebookSyncPlugin extends Plugin {
	...
	private settingTab: NotebookSettingTab | null = null;
	...
	async onload() {
		...
		this.api = new NotebookApiClient({
		...
		this.settingTab = new NotebookSettingTab(this.app, this);
		...
		this.addCommand({
			...
			callback: async () => {
				try {
					...
					new Notice("Open Notebook: re-sync started.");
				} catch (err) {
					new Notice(`Open Notebook: re-sync failed — ${(err as Error).message}`);
				}
			},
		});
	...
	private notifyAuthError(): void {
		...
		new Notice("Open Notebook: API token expired or invalid. Paste a fresh token in settings.", 8000);
	}

	async loadSettings() {
		const data = (await this.loadData()) as Partial<NotebookSyncSettings> | null;
		this.settings = {
			...DEFAULT_SETTINGS,
			...(data ?? {}),
